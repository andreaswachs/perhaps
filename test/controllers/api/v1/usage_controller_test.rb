...
class Api::V1::UsageControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should work correctly when approaching rate limit" do
    # Freeze time to avoid hour boundary issues - rate limiter uses hourly windows
    # If test runs across an hour boundary, requests get counted in different buckets
    travel_to Time.zone.local(2024, 6, 15, 10, 30, 0) do
