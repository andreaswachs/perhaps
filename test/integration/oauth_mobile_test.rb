...
class OauthMobileTest < ActionDispatch::IntegrationTest
  setup do
    ...
    @oauth_app = Doorkeeper::Application.create!(
      name: "Perhaps Mobile App",
      redirect_uri: "perhapsapp://oauth/callback",
      scopes: "read"
    )
  end
  ...
  test "mobile oauth authorization with custom scheme redirect" do
    ...
    assert_match(/perhapsapp:\/\/oauth\/callback/, response.body)
  end
  ...
  test "mobile oauth authorization flow completes successfully" do
    ...
    assert response.location.start_with?("perhapsapp://oauth/callback")
  end
