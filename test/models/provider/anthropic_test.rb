...
class Provider::AnthropicTest < ActiveSupport::TestCase
  ...
  test "provides available models with configuration" do
    models = Provider::Anthropic.available_models

    assert_equal 2, models.size

    fast_model = models.find { |m| m[:tier] == :fast }
    intelligent_model = models.find { |m| m[:tier] == :intelligent }

    assert_equal "Fast", fast_model[:name]
    assert_equal "claude-haiku-4-5-20250929", fast_model[:id]

    assert_equal "Intelligent", intelligent_model[:name]
    assert_equal "claude-sonnet-4-5-20250929", intelligent_model[:id]
  end

  test "supports both fast and intelligent models" do
    assert @subject.supports_model?("claude-haiku-4-5-20250929")
    assert @subject.supports_model?("claude-sonnet-4-5-20250929")
    assert_not @subject.supports_model?("invalid-model")
  end

  test "anthropic errors are automatically raised" do
    VCR.use_cassette("anthropic/chat/error") do
      response = @anthropic.chat_response("Test", model: "invalid-model-that-will-trigger-api-error")
  ...
  test "auto detects merchants" do
    VCR.use_cassette("anthropic/auto_detect_merchants") do
      input_transactions = [
  ...
  test "basic chat response" do
    VCR.use_cassette("anthropic/chat/basic_response") do
  ...
  test "streams basic chat response" do
    VCR.use_cassette("anthropic/chat/basic_streaming_response") do
      collected_chunks = []
      ...
      assert_includes text_chunks.map(&:data).join, "Yes"
      assert_includes response_chunks.first.data.messages.first.output_text, "Yes"
  ...
  test "streams chat response with function calls" do
    VCR.use_cassette("anthropic/chat/streaming_function_calls") do
      collected_chunks = []
