...
#
# 1. Load a scrubbed production-like slice of data into perhaps_benchmarking DB locally
# 2. Setup .env.production so that the Rails app can boot with RAILS_ENV=production and connect to local perhaps_benchmarking DB
...
namespace :benchmarking do
  ...
  task code: :environment do
    Benchmark.ips do |x|
      ...
      family = User.find_by(email: "user@perhaps.local").family
